...
runtime. The canonical runtime uses DirectoryIngestionService with SQLite
and deterministic vector embeddings.
"""
...
SKIP_DIRS = {
    ...
    ".next", "coverage", ".mempalace", ".ruff_cache", ".mypy_cache", ".pytest_cache",
    ".cache", ".tox", ".nox", ".idea", ".vscode", ".ipynb_checkpoints", ".eggs",
    "htmlcov", "target",
}

SKIP_FILENAMES = {
    "mempalace.yaml", "mempal.yml", ".gitignore", "package-lock.json",
    "mempalace.yml", "mempal.yml",
}
...
def process_file(
    ...
    for chunk in chunks:
        added = add_drawer(
            collection=collection, wing=wing, room=room,
            content=chunk["content"], source_file=source_file,
            chunk_index=chunk["chunk_index"], agent=agent,
        )
        if added:
            drawers_added += 1
...
def mine(
    ...
    files = scan_project(project_dir, respect_gitignore=respect_gitignore, include_ignored=include_ignored)
    if limit > 0:
        files = files[:limit]
    ...
    for i, filepath in enumerate(files, 1):
        drawers = process_file(
            filepath=filepath, project_path=project_path, collection=collection,
            wing=wing, rooms=rooms, agent=agent, dry_run=dry_run,
        )
